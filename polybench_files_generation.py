...
def generate_function_call(generated_code, kernel_name, code_vars, code_arrays, folder):
    ...
    function_body = get_body(kernel_name)
...
def generate_header(kernel_name):
    header_code = ['#ifndef _{}_H'.format(kernel_name), '#define _{}_H'.format(kernel_name),
                   '# if !defined(LARGE_DATASET) && !defined(EXTRALARGE_DATASET)', '# define LARGE_DATASET', '# endif',
                   '# ifdef LARGE_DATASET', '# define xA 128', '# define yA 128', '# define zA 128', '#endif',
                   header_string]
    return header_code
